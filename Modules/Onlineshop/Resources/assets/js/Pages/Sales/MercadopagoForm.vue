<script setup>
    ...
    const payerEmail = computed(() => {
        return props.personInvoice?.email || page.props.auth?.user?.email || '';
    });
    ...
    const handlePaymentResponse = (data) => {
        if (data.status === "approved") {
            ...
            showAlertToast("El curso ya estÃ¡ disponible.","success");
            ...
            showAlertToast(data.message || "Pago no aprobado.","error");
    ...
    const processYapePayment = async () => {
        if (!mp) {
            showAlertToast("Mercado Pago todavÃ­a no estÃ¡ listo.", "error");
            return;
        }

        const email = yapeForm.email || payerEmail.value;
        if (!yapeForm.phoneNumber || !yapeForm.otp || !email) {
            showAlertToast("Ingresa celular, cÃ³digo OTP y correo.", "error");
            return;
        }
        ...
        try {
            ...
            const response = await axios({
                ...
                data: {
                    ...
                    transaction_amount: Number(totalSale.value),
                    installments: 1,
                    payment_method_id: 'yape',
                    payer: { email },
                    payer_email: email,
                    products: itemsCart.value,
            ...
            const message = error?.response?.data?.error || error.message || "Error al procesar Yape.";
            showAlertToast(message, "error");
        } finally {
                            ...
                            <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                                ...
                                <div class="mt-3 space-y-2">
                                    ...
                                    <div class="rounded-lg border border-gray-200 shadow-sm dark:border-gray-700">
                                        <button type="button"
                                            class="flex w-full items-center justify-between rounded-lg bg-white px-4 py-3 text-left text-sm font-semibold text-gray-900 hover:bg-gray-50 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700"
                                        ...
                                        <div v-show="activeAccordion === 'yape'" class="border-t border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <div class="rounded-lg border border-gray-200 shadow-sm dark:border-gray-700">
                                        ...
                                        <div v-show="activeAccordion === 'card'" class="border-t border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
