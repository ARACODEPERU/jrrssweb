...
const opemModalDetails = async (sales) => {
    if (
        sales.invoice_status === "registrado" ||
        sales.invoice_status === "Rechazada" ||
        sales.invoice_status === "Pendiente"
    ) {
...
const sendSunatDocument = (document) => {
    Swal.fire({
        ...
        preConfirm: () => {
            return axios
                .get(
                    route("saledocuments_send", [
                        document.document_id,
                        document.invoice_type_doc,
                    ])
                )
                .then((res) => {
                    if (!res.data.success) {
...
const getAffectation = (id) => {
    const selectedAffectation = props.affectations.find(
        (affectation) => affectation.id === id
    );
...
const getUnitTypes = (id) => {
    const selectedUnitType = props.unitTypes.find(
        (unitType) => unitType.id === id
    );
    //console.log(selectedAffectation.description)
    if (selectedUnitType) {
...
const saveChangesDetails = () => {
    ...
    axios
        .post(route("saledocuments_update_details"), documentDetails.value)
        .then((response) => {
...
const createFormReason = () => {
    ...
    rInput.classList.add("form-textarea");
        ...
        if (result.value) {
            Swal.fire({
                ...
                preConfirm: async (input) => {
                    let textarea =
                        document.getElementById("ctnTextareaReason").value;
                    let resp = null;
                    if (textarea) {
                        resp = axios
                            .post(route("saledocuments_cancel_document"), {
                                reason: textarea,
                                id: item.document_id,
...
<template>
    <AppLayout title="Documentos">
        ...
        <div class="mt-5">
                ...
                <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    <div class="flex gap-3">
                        <Keypad>
                            <template #botones>
                                <Link
                                    :href="route('sale_credit_notes_create')"
            ...
            <div class="panel pb-1.5 mt-6">
                <DataTable
                    ref="documentTable"
                    :options="options"
                    :ajax="route('sale_credit_notes_table')"
                    :columns="columns"
                >
                        ...
                        <div class="flex gap-4 items-center justify-center">
                            <div class="dropdown">
                                ...
                                >
                                    <button
                                        type="button"
                                        class="btn btn-outline-primary px-2 py-2 dropdown-toggle"
                                    >
                    ...
                    <template #status="props">
                        <div>
                            <span
                                ...
                                class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-yellow-300 border border-yellow-300"
                                >Activa</span
                            >
                            <span
                                v-else-if="props.rowData.status == 3"
        ...
        <ModalLargeX
            ...
            :icon="'/img/bienes.png'"
        >
            <template #title> Detalles de la venta </template>
            <template #message>
            ...
            <template #content>
                <div class="relative overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody>
                            <tr
                                v-for="(item, ko) in documentDetails.items"
                                class="bg-white border-b border-stroke dark:bg-gray-900 dark:border-gray-700"
                            >
                                <th
                                    scope="row"
                                ...
                                <td class="px-4 py-2">
                                    <span v-show="!showteButtonSave">{{
                                        item.decription_product
                                    }}</span>
                                    <textarea
                                        v-show="showteButtonSave"
                                        v-model="item.decription_product"
                                        class="form-textarea"
                                        rows="3"
                                    ></textarea>
                                </td>
                                ...
                                <td style="display: none" class="px-4 py-2">
                                    <span v-show="!showteButtonSave">{{
                                        getAffectation(item.type_afe_igv)
                                    }}</span>
                                    <select
                                        v-show="showteButtonSave"
                                        v-model="item.type_afe_igv"
                                        class="form-select form-select-sm"
                                    >
                                        <option
                                            v-for="(row, ci) in affectations"
                                            :value="row.id"
                                        >
                                            {{ row.description }}
                                        </option>
            ...
            <template #buttons>
                ...
                <PrimaryButton
                    v-if="showteButtonSave"
                    class="mr-2"
                    :class="{ 'opacity-25': displayLoaderDetails }"
                    :disabled="displayLoaderDetails"
                    @click="saveChangesDetails"
                >
                    <svg
                        v-show="displayLoaderDetails"
                        aria-hidden="true"
                        role="status"
        ...
        <DialogModal
            :show="displayEditDocument"
            @close="closeModalEditDocument"
        >
            <template #title>
                <div class="sm:flex sm:items-start">
                    <div
                        class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-100 dark:bg-green-800 sm:mx-0 sm:h-10 sm:w-10"
                    >
            ...
            <template #content>
                <div class="grid grid-cols-4 gap-4">
                    <div class="col-span-6 sm:col-span-1">
                        <InputLabel
                            for="client_number"
                            value="Cliente Num. Doc."
                        />
                        <TextInput
                            id="client_number"
                            v-model="formHead.client_number"
                            type="text"
                            class="block w-full mt-1"
                            autofocus
                        />
                        <InputError
                            :message="formHead.errors.client_number"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel
                            for="client_rzn_social"
                            value="Cliente Razon Social"
                        />
                        <TextInput
                            id="client_rzn_social"
                            v-model="formHead.client_rzn_social"
                            type="text"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.client_rzn_social"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <TextInput
                            id="client_address"
                            v-model="formHead.client_address"
                            type="text"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.client_address"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-4">
                        <InputLabel
                            for="client_ubigeo_description"
                            value="Departamento-Provincia-Distrito"
                        />
                        <TextInput
                            id="client_ubigeo_description"
                            v-model="formHead.client_ubigeo_description"
                            type="text"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.client_ubigeo_code"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <TextInput
                            id="client_phone"
                            v-model="formHead.client_phone"
                            type="text"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.client_phone"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <TextInput
                            id="client_email"
                            v-model="formHead.client_email"
                            type="text"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.client_email"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-2">
                        <InputLabel
                            for="invoice_broadcast_date"
                            value="Fecha Emisión"
                        />
                        <TextInput
                            id="invoice_broadcast_date"
                            v-model="formHead.invoice_broadcast_date"
                            type="date"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.invoice_broadcast_date"
                            class="mt-2"
                        />
                    </div>
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <TextInput
                            id="invoice_due_date"
                            v-model="formHead.invoice_due_date"
                            type="date"
                            class="block w-full mt-1"
                        />
                        <InputError
                            :message="formHead.errors.invoice_due_date"
                            class="mt-2"
                        />
                    </div>
                </div>
            ...
            <template #footer>
                <PrimaryButton class="mr-2" @click="saveHeadDocument">
                    Guardar Cambios
                </PrimaryButton>
