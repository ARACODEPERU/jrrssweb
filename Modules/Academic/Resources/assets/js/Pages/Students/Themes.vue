                    ...
                    <template v-if="module.teacher_id">
                        <div class="flex items-center gap-x-3 max-w-xl">
                            <div class="shrink-0">
                                ...
                                <img v-else :src="`https://ui-avatars.com/api/?name=${module?.teacher?.person?.names ?? 'T'}&size=150&rounded=true`" class="shrink-0 size-16 rounded-full" alt="avatar"/>
                            </div>
                    ...
                    <template v-else>
                        <div v-if="course.teacher" class="flex items-center gap-x-3 max-w-xl">
                            <div class="shrink-0">
                                ...
                                <img v-else :src="`https://ui-avatars.com/api/?name=${course?.teacher?.person?.names ?? 'T'}&size=150&rounded=true`" class="shrink-0 size-16 rounded-full" alt="avatar"/>
                            </div>
                    ...
                    <div class="flex flex-col mt-5 gap-4 text-sm">
                        <template v-for="(theme, index) in module.themes">
                            <div @click="selectTheme(theme)" class="cursor-pointer flex justify-between items-center p-3 rounded-sm shadow-sm hover:bg-white-dark/10 dark:hover:bg-[#181F32] font-medium ltr:hover:pl-3 rtl:hover:pr-3 duration-300 dark:bg-gray-700 dark:text-white"
                                ...
                                <span class="font-bold text-yellow-500">{{ theme.contents?.length ?? 0 }}</span>
                            </div>
                        </template>
                    </div>
                </div>
                                        ...
                                        <div v-for="(comment, ibex) in commentsData" class="mt-8">
                                            <div class="flex align-top">
                                                <div class="shrink-0">
                                                            ...
                                                            <img v-else :src="'https://ui-avatars.com/api/?name='+(comment?.user?.name ?? 'X')+'&size=150&rounded=true'" class="p-1 rounded-full w-14 h-14 ring-2 ring-gray-100/20" :alt="comment?.user?.name"/>
                                                </div>
